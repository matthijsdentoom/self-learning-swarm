...
#include <zebroscrimmage/plugins/metrics/RewardMetric/RewardMetric.h>
...
REGISTER_PLUGIN(scrimmage::Metrics,
                scrimmage::metrics::RewardMetric,
                RewardMetric_plugin)

namespace scrimmage {
namespace metrics {

    RewardMetric::RewardMetric() : Metrics() {
        print_team_summary_ = false;
    }

    void RewardMetric::init(std::map<std::string, std::string> &/*params*/) {
    ...
    void RewardMetric::print_team_summaries() {
    ...
    void RewardMetric::calc_team_scores() {

        CSV csv;
        std::string filename = parent_->mp()->log_dir() + "/rewards.csv";

        std::cout << "doing team scores to: " << filename << std::endl;
